...
class Wall(object):
    def __init__(self, orient='x', axis_pos=0.0,
                 endpoints=(-1, 1, -1, 1), width=0.025, hard=True):
...
class World(object):
    ...
    @property
    def entities(self):
        return self.agents + self.landmarks + self.dummy_agents
    ...
    def apply_action_force_rotation(self, p_force_rotation):
        ...
        for i, agent in enumerate(self.agents):
            if agent.movable:
                noise = np.random.randn(
                    *agent.action.r.shape) * agent.r_noise if agent.r_noise else 0.0
