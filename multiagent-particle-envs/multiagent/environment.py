...
class MultiAgentEnv(gym.Env):
    ...
    def __init__(self, world, reset_callback=None, reward_callback=None,
        ...
        for agent in self.agents:
            ...
            #     TODO: add action_space.n
                setattr(self.action_space[-1], 'n', u_action_space.n +
                        rotation_action_space.n + audio_action_space.n)
            else:
    ...
    def step(self, action_n, use_handcraft_policy=False):
        ...
        return obs_n, reward_n, done_n, info  # , info_n
    ...
    def _set_action(self, action, agent, action_space, time=None):
        ...
        if np.abs(rotation_encoding[0] - 1) < 1e-5:
            agent.action.r[0] = -rotate_unit
        if np.abs(rotation_encoding[1] - 1) < 1e-5:
            agent.action.r[0] = +rotate_unit
        if np.abs(rotation_encoding[2] - 1) < 1e-5:
            agent.action.r[1] = -rotate_unit
        if np.abs(rotation_encoding[3] - 1) < 1e-5:
            agent.action.r[1] = +rotate_unit
        ...
        if np.abs(audio_encoding[0] - 1) < 1e-5:
            agent.action.audio = None
        if np.abs(audio_encoding[1] - 1) < 1e-5:
            agent.action.audio = AudioAction.HandsUp
        if np.abs(audio_encoding[2] - 1) < 1e-5:
            agent.action.audio = AudioAction.Freeze
    ...
    def _render(self, mode='human', close=True):
        ...
        if self.render_geoms is None:
            ...
            for wall in self.world.walls:
                corners = ((wall.axis_pos -
                            0.5 *
                            wall.width, wall.endpoints[0]), (wall.axis_pos -
                                                             0.5 *
                                                             wall.width, wall.endpoints[1]), (wall.axis_pos +
                                                                                              0.5 *
                                                                                              wall.width, wall.endpoints[1]), (wall.axis_pos +
                                                                                                                               0.5 *
                                                                                                                               wall.width, wall.endpoints[0]))
                if wall.orient == 'H':
        ...
        for i in range(len(self.viewers)):
            ...
            for e, entity in enumerate(self.world.agents):
                ...
                self.render_geoms_xform[ind].set_translation(
                    *entity.state.p_pos)
                self.render_geoms_xform[ind].set_rotation(
