...
class AttentionCritic(nn.Module):
    ...
    def forward(self, inps, agents=None, return_q=True, return_all_q=False,
        ...
        sa_encodings = [
            encoder(inp) for encoder, inp in zip(
                self.critic_encoders, inps)]
        ...
        # extract selectors for each head for each agent that we're returning Q
        # for
        all_head_selectors = [[sel_ext(enc) for i, enc in enumerate(
            s_encodings) if i in agents] for sel_ext in self.selector_extractors]
        ...
        for curr_head_keys, curr_head_values, curr_head_selectors in zip(
            ...
            for i, a_i, selector in zip(
                ...
                values = [v for j, v in enumerate(
                    curr_head_values) if j != a_i]
                # calculate attention across agents
                attend_logits = torch.matmul(selector.view(
                    selector.shape[0], 1, -1), torch.stack(keys).permute(1, 2, 0))
                # scale dot-products by size of key (from Attention is All You
                # Need)
                ...
                logger.add_scalars(
                    ...
                    dict(
                        ('head%i_entropy' %
                         h_i,
                         ent) for h_i,
                        ent in enumerate(head_entropies)),
                    niter)
            if len(agent_rets) == 1:
