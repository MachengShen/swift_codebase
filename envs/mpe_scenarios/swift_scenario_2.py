        ...
        for i, agent in enumerate(self.agents):
            ...
            for room in self.rooms:
                for cell in room.cells:
                    ...
                    if agent.check_within_fov(cell_center) and doIntersect(
                            cell_center, Point(agent.state.p_pos), room.window.p1, room.window.p2):
...
class FieldOfView(object):
    ...
    def check_within_fov(self, p_in):  # check if a point p is within fov
        ...
        if np.linalg.norm(vector1) > self.sensing_range:
            return False
        vector2 = np.array([np.cos(self._attached_agent.state.boresight), np.sin(
            self._attached_agent.state.boresight)]).squeeze()
...
class Scenario(BaseScenario):
    ...
    def _set_rooms(self, world, num_room, arena_size):
        ...
        dummy_x_index = np.kron(
            np.arange(
                self.max_room_num_per_dim) * 2 + 1,
            np.ones(
                self.max_room_num_per_dim))
        dummy_y_index = np.kron(
            np.ones(
                self.max_room_num_per_dim), np.arange(
                self.max_room_num_per_dim) * 2 + 1)
        dummy_xy_index = np.array(
            [dummy_x_index, dummy_y_index]).astype(int).transpose()
        for i in range(self.num_room):
            index = np.where((dummy_xy_index == (
                xy_index[i, 0], xy_index[i, 1])).all(axis=1))
            dummy_xy_index = np.delete(dummy_xy_index, index, axis=0)

        assert dummy_xy_index.shape[0] + \
            xy_index.shape[0] == self.max_room_num_per_dim ** 2
        # length = arena_size / num_room
        room_centers = np.array(
            [[-arena_size / 2 + self.room_length / 2 * i, -arena_size / 2 + self.room_length / 2 * j]
             for i, j in zip(xy_index[:, 0], xy_index[:, 1])])
        world.rooms = [Room(Point(room_centers[i, :]), self.room_length,
                            self.room_length) for i in range(num_room)]
    ...
    # world.room_centers = room_centers
        ...
        world.dummy_rooms = [Room(Point(dummy_room_centers[i, :]), self.room_length,
                                  self.room_length) for i in range(dummy_xy_index.shape[0])]
    ...
    def _set_walls(self, world, num_room, arena_size):
        ...
        for i in range(num_room):
            wall_axis_pos[4 *
                          i:4 *
                          i +
                          4] = np.array([room_centers[i, 1] +
                                         window_length, room_centers[i, 0] -
                                         window_length, room_centers[i, 1] -
                                         window_length, room_centers[i, 0] +
                                         window_length])
            wall_endpoints.append(
                (room_centers[i, 0] - window_length, room_centers[i, 0] + window_length))
            wall_endpoints.append(
                (room_centers[i, 1] - window_length, room_centers[i, 1] + window_length))
            wall_endpoints.append(
                (room_centers[i, 0] - window_length, room_centers[i, 0] + window_length))
            wall_endpoints.append(
                (room_centers[i, 1] - window_length, room_centers[i, 1] + window_length))
        ...
        boundary_wall_axis_pos = np.array(
            [-arena_size_larger / 2, -arena_size_larger / 2, arena_size_larger / 2, arena_size_larger / 2])
        boundary_wall_endpoints = []
    ...
    def _reset_blue_states(self, world):
        ...
        for agent in world.agents:
            ...
            agent.state.boresight = np.array(
    ...
    def observation(self, agent, world):
        ...
        for room in world.dummy_rooms:
            for cell in room.cells:
                ...
                dummy_cell_info.extend(
                    [cell_pos, fov_flag, cell.get_cell_state_encoding(), np.array([-1.])])
