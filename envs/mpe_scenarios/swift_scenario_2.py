...
class Scenario(BaseScenario):
	...
	def _set_walls(self, world, num_room, arena_size):
		...
		room_centers = world.room_centers
		for i in range(num_room):
			wall_axis_pos[4 * i:4 * i + 4] = np.array(
		...
		world.walls = [Wall(orient=wall_orient[i], axis_pos=wall_axis_pos[i], endpoints=wall_endpoints[i]) for i in
					   range(num_wall)]
	...
	def observation(self, agent, world):
		...
		output = np.concatenate([agent.state.p_vel] + [agent.state.p_pos] + [
			agent.state.boresight] + other_pos + other_vel + other_heading + cell_info + dummy_cell_info)
		# print(output)
		return output
