...
class Scenario(BaseScenario):
    ...
    def _set_rooms(self, world, num_room, arena_size):
        ...
        agent_room_centers = np.array([[-arena_size / 2 + self.room_length / 2 * i, -arena_size /
                                        2 + self.room_length / 2 * j] for i, j in zip(xy_index[:, 0], xy_index[:, 1])])
        ...
        all_room_centers = np.array([[-arena_size / 2 + self.room_length / 2 * i, -arena_size /
                                      2 + self.room_length / 2 * j] for i, j in zip(xy_index_all[:, 0], xy_index_all[:, 1])])
