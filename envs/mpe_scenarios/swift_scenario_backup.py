...
class Scenario(BaseScenario):
    ...
    def _set_walls(self, world, num_room, arena_size):
        ...
        for i in range(num_room):
            ...
            wall_endpoints.append(
                (-arena_size / 2 + length * i, -arena_size / 2 + length * (i + 1)))
            wall_endpoints.append((arena_size / 2, arena_size / 2 - length))
            wall_endpoints.append(
                (-arena_size / 2 + length * i, -arena_size / 2 + length * (i + 1) - window_length))
        ...
        boundary_wall_axis_pos = np.array(
            [-arena_size_larger / 2, -arena_size / 2, arena_size_larger / 2, arena_size / 2])
        boundary_wall_endpoints = []
        boundary_wall_endpoints.append((-arena_size / 2, arena_size / 2))
    ...
    def _set_rooms(self, world, num_room, arena_size=2.0):
        length = arena_size / num_room
        room_centers = np.array([[-arena_size / 2 + length / 2 + i * \
                                length, arena_size / 2 - length / 2] for i in range(num_room)])
        world.rooms = [Room(Point(room_centers[i, :]), length, length)
                       for i in range(num_room)]
