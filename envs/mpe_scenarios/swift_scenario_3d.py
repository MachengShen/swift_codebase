		...
		for i, agent in enumerate(self.agents):
			...
			for room in self.rooms:
				for cell in room.cells:
					...
					cell_pos = np.array([cell_center.x, cell_center.y, cell_center.z])
					flag_1 = agent.check_within_fov(cell_pos)
					flag_2 = doIntersect(agent.state.p_pos, cell_pos, room.window)
...
class Room_cell(object):
	...
	def add_agent(self, agent: DummyAgent):
		...
		self._occupant_agent.state.p_pos = np.array([self._center.x, self._center.y, self._center.z])
...
class FieldOfView(object):
	...
	def __init__(self, attached_agent, half_view_angle=np.pi/4, sensing_range=2):
	...
	def check_within_fov(self, p_in): #check if a point p is within fov
		...
		vector2 = np.array([prj_norm*np.cos(self._attached_agent.state.boresight[0]),
							prj_norm*np.sin(self._attached_agent.state.boresight[0]),
							np.sin(self._attached_agent.state.boresight[1])]).squeeze()
...
class Scenario(BaseScenario):
	...
	def _set_rooms(self, world, num_room, arena_size):
		...
		for i in range(self.n_floor):
			real_room_index[i*self.num_room:(i+1)*self.num_room] = row_index + i*xy_index_all.shape[0]
		...
		world.agent_rooms = [world.rooms[row_index_] for row_index_ in world.real_room_index]
		# world.room_centers = room_centers
		# world.x_index = x_index
		#
	...
	def _set_room_windows(self, world, num_room, arena_size):
		...
		for i, room in enumerate(world.rooms):
			if x_index_all[i] == 1:
				...
				endpoints = (all_room_centers[i, 1] - self.room_length / 2., all_room_centers[i, 1] + self.room_length / 2.,
							 all_room_centers[i, 2], all_room_centers[i, 2] + window_height)
				...
				endpoints = (
				all_room_centers[i, 0] - self.room_length / 2., all_room_centers[i, 0] + self.room_length / 2.,
				all_room_centers[i, 2], all_room_centers[i, 2] + window_height)
	...
	def _set_walls(self, world, num_room, arena_size):
		...
		for i in range(num_room * self.n_floor):
			...
			wall_axis_pos[6*i:6*i+6] = np.array([room_center_[0] + self.room_length / 2., room_center_[0] - self.room_length / 2.,
												 room_center_[1] + self.room_length / 2., room_center_[1] - self.room_length / 2.,
												 room_center_[2] + self.floor_height / 2., room_center_[2] - self.floor_height / 2.])
			# l += n * [v]
		...
		boundary_wall_axis_pos = np.array([room_center_[0] + arena_size_larger / 2., room_center_[0] - self.arena_size / 2.,
											room_center_[1] + self.arena_size / 2., room_center_[1] - arena_size_larger / 2.,
											room_center_[2] + self.floor_height * (self.n_floor+1) / 2., room_center_[2] - self.floor_height * self.n_floor / 2.])
		boundary_wall_endpoints = []
		endpoint_x = (room_center_[1] - arena_size_larger / 2.,
					  room_center_[1] + self.arena_size / 2.,
					  room_center_[2] - self.floor_height * self.n_floor / 2.,
					  room_center_[2] + self.floor_height * (self.n_floor+1) / 2.)
		endpoint_y = (room_center_[0] - self.arena_size / 2.,
	...
	def _reset_blue_states(self, world):
		...
		for agent in world.agents:
			...
			agent.state.boresight = np.random.uniform(-np.pi, +np.pi, size=world.dim_p-1)
	...
	def observation(self, agent, world):
		...
		for i, room in enumerate(world.rooms):
			if i in world.real_room_index:
				for cell in room.cells:
					cell_pos = np.array([cell._center.x, cell._center.y, cell._center.z])
					flag_1 = agent.check_within_fov(cell_pos)
					flag_2 = doIntersect(agent.state.p_pos, cell_pos, room.window)
					fov_flag = encode_boolean(flag_1 and flag_2)
					cell_info.extend([cell_pos, fov_flag, cell.get_cell_state_encoding(), cell.get_belief()])
			else:
				cell_info.extend([-np.ones(3), -np.ones(2), -np.ones(3), -np.ones(1)])
